...
class BaseWrapper(gym.Env):
    ...
    def __init__(self, env, teacher, discount_factor, context_visible, reward_from_info=False,
                 context_post_processing=None, episodes_per_update=50, episodes_per_aux_update=0, aux_teacher=None):
        gym.Env.__init__(self)

        # CEM extension
        self.episodes_per_update = episodes_per_update
        self.episodes_per_aux_update = episodes_per_aux_update
    ...
    def reset(self):
        if self.cur_context is None:
            if self.use_teacher:
                ...
                self.cur_context = self.aux_teacher.sample()
    ...
    def update(self, step):
        ...
        if step[2]:
            ...
            # CEM extension
            self.done_callback(step, self.cur_initial_state.copy(), self.cur_context, self.discounted_reward,
                               self.undiscounted_reward, use_teacher=self.use_teacher)
            if self.use_teacher:
                self.stats_buffer.update_buffer((self.undiscounted_reward, self.discounted_reward, self.step_length))
                self.context_trace_buffer.update_buffer((self.undiscounted_reward, self.discounted_reward,
                                                         self.processed_context.copy()))
            else:
                self.aux_stats_buffer.update_buffer((self.undiscounted_reward, self.discounted_reward,
                                                     self.step_length))
                self.aux_context_trace_buffer.update_buffer((self.undiscounted_reward, self.discounted_reward,
                                                             self.processed_context.copy()))
            self.episodes_counter += 1
            if self.episodes_counter >= (self.episodes_per_update + self.episodes_per_aux_update):
                self.episodes_counter = 0
                self.use_teacher = True
            elif self.episodes_counter >= self.episodes_per_update:
