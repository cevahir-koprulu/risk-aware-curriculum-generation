...
class CauchyTorchDistribution(TorchDistribution):
    ...
    def __init__(self, mu, chol_flat, use_cuda, dtype=torch.float32):
        ...
        self.distribution_t = MultivariateStudentT(df=1, loc=self._mu,
                                                   scale_tril=self.to_tril_matrix(self._chol_flat, self._dim))

    def __copy__(self):
        return CauchyTorchDistribution(self._mu, self._chol_flat, self.use_cuda)

    def __deepcopy__(self, memodict=None):
        return CauchyTorchDistribution(copy.deepcopy(self._mu), copy.deepcopy(self._chol_flat), self.use_cuda)
    ...
    def mean_t(self):
        ...
        return self.distribution_t.loc
    ...
    def from_weights(dim, weights, use_cuda=False, dtype=torch.float32):
        ...
        return CauchyTorchDistribution(mu, chol_flat, use_cuda=use_cuda, dtype=dtype)
